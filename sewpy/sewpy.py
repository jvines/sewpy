...
defaultparams = ["XWIN_IMAGE", "YWIN_IMAGE", "AWIN_IMAGE", "BWIN_IMAGE",
...
class SEW():
    ...
    def __init__(self, workdir=None, sexpath="sex", params=None, config=None, configfilepath=None, nice=None, loglevel=None):
        ...
        if workdir is not None:
            ...
            if os.path.isdir(workdir):
                ...
                logger.info("Making new SExtractor workdir '%s'..." %
                            (workdir))
                os.makedirs(workdir)
        else:
    ...
    def get_version(self):
        """
        Calls sextractor it without arguments and parses the stdout.

        :returns: a string (e.g. '2.4.4')
        """
        try:
            p = subprocess.Popen(
                [self.sexpath], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        except:
            raise RuntimeError(
                "Could not run SExtractor. Is the path '%s' correct ? If not, specify sexpath='/path/to/sextractor'" % self.sexpath)
        out, err = p.communicate()
        version_match = re.search(
            "[Vv]ersion ([0-9\.])+", err.decode(encoding='UTF-8'))
    ...
    def _check_params(self):
        ...
        for param in self.params:
            ...
            if cleanparam not in self.fullparamlist:
                logger.warning(
                    "Parameter '%s' seems strange and might be unknown to SExtractor"
                    % (param))
    ...
    def _set_instance_config(self):
        ...
        Called by __init__(). If needed, you could still mess with this config
        after __init__() has run.
        """

        if "PARAMETERS_NAME" in self.config.keys():
            logger.info(
                "You specified your own PARAMETERS_NAME, I will use it.")
        else:
        ...
        if "CATALOG_NAME" in self.config.keys():
            logger.warning(
                "You specified your own CATALOG_NAME, but I will *NOT* use it!"
            )
    ...
    def _write_default_config(self, force=False):
        ...
        if force or not os.path.exists(self._get_config_filepath()):
            p = subprocess.Popen(
                [self.sexpath, "-dd"], stdout=subprocess.PIPE,
            ...
            logger.debug(
                "Default config file already exists, I don't overwrite it.")
...
1 2 1""")
            ...
            logger.debug(
                "Default conv file already exists, I don't overwrite it.")
    ...
    def _write_default_psf(self):
        ...
        if not os.path.exists(self._get_psf_filepath()):
            arr = np.array(
                [[[2.40381883e-06,   7.60069597e-05,   8.54981554e-05,
            ...
            tbhdu = fits.BinTableHDU.from_columns(fits.ColDefs(
                [fits.Column(name='PSF_MASK', format='961E', dim='(31, 31, 1)',
                             array=arr)]))
            hdr = tbhdu.header
            ...
            logger.debug(
                "Default psf file already exists, I don't overwrite it.")
    ...
    def _clean_workdir(self):
        ...
        ), self._get_conv_filepath(), self._get_psf_filepath()]
    ...
    def _write_assoc(self, cat, xname, yname, imgname):
        """
        Writes a plain text file which can be used as sextractor input for the
        ...
        f.writelines(lines)
        f.close()
        logger.debug("Wrote ASSOC file %s..." %
                     (self._get_assoc_filepath(imgname)))
    ...
    def __call__(self, imgfilepath, imgname=None, assoc_cat=None, assoc_xname="x", assoc_yname="y",
        ...
        # We make a deep copy of the config, that we can modify with settings
        # related to this particular image.
        ...
        if os.path.exists(self._get_cat_filepath(imgname)):
            logger.warning(
                "Output catalog %s already exists, I will overwrite it" % (
        ...
        if assoc_cat is not None:
            ...
            if "VECTOR_ASSOC_2" in assoc_cat.colnames:
                raise RuntimeError(
                    "Do not give me an assoc_cat that already contains a column VECTOR_ASSOC_2")
            ...
            if "ASSOC_RADIUS" not in imgconfig:
                logger.warning(
                    "ASSOC_RADIUS not specified, using a default of 10.0")
                imgconfig["ASSOC_RADIUS"] = 10.0
            if "ASSOC_TYPE" not in imgconfig:
                logger.warning(
                    "ASSOC_TYPE not specified, using a default NEAREST")
                imgconfig["ASSOC_TYPE"] = "NEAREST"
            if "ASSOCSELEC_TYPE" in imgconfig:
                raise RuntimeError(
                    "Sorry, you cannot mess with ASSOCSELEC_TYPE yourself when using the helper. I'm using MATCHED.")
        ...
        popencmd = [self.sexpath, imgfilepath,
                    "-c", self._get_config_filepath()]
        if self.nice is not None:  # We prepend the nice command
        ...
        # And we run
        logger.info("Starting SExtractor now, with niceness %s..." %
                    (self.nice))
        logger.debug("Running with command %s..." % (popencmd))
        p = subprocess.Popen(
            popencmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        out, err = p.communicate()
        ...
        if writelog:
            ...
            logfile.write("\n".join(
                ["%30s : %30s" % (str(key), str(value)) for (key, value) in imgconfig.items()]))
            logfile.write("\n\n####### stdout #######\n")
        ...
        if returncat:
            if assoc_cat is None:
                ...
                if not len(intable) == len(joined):
                    raise RuntimeError(
                        "Problem with joined tables: intable has %i rows, joined has %i. %s %s" % (
                            len(intable), len(joined),
                ...
                if joined.masked:
                    logger.info(
                        "ASSOC join done, my output is a masked table.")
                    joined[prefix + "assoc_flag"] = joined[joined.colnames[-1]
                                                           ].mask == False
                    ...
                    logger.info(
                        "ASSOC join done, I could find all your sources, my output is not masked.")
...
"""
    ...
    fullparamlist = list(
        map(lambda s: s[1:-1], re.compile("#\w*\s").findall(fullparamtxt)))
